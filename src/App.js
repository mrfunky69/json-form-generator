...
import './App.css';
import Jsonform1 from './jsonForm'
//import Jsonform1 from './dummy';
...
// ]`);
  ...
  try {
    jsoN = JSON.parse(textAreaValue);
  } catch (e) {
    console.log(e)
  }


  const [s, setS] = useState(jsoN);
  console.log(textAreaValue, ' <- text ')
  ...
  return (
    <>
      <row>
        ...
        ></textarea>
        <button  className='button-33'onClick={(e) => {
          if (textAreaValue !== '') {
            try {
              let json = JSON.parse(textAreaValue);
              setLoading(true);
              setTimeout(() => { setS(json); setLoading(false); }, 20);
            } catch (e) {
              console.log(e);
            }
          }
      ...
      <row>
        <div className="App">
          {!loading ? <Jsonform1 formJson={s} /> : null}
        </div>
      </row>
    </>
